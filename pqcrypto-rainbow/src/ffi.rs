...
#[cfg(enable_avx2)]
pub const PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_SECRETKEYBYTES: usize = 92960;
#[cfg(enable_avx2)]
pub const PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_PUBLICKEYBYTES: usize = 58144;
#[cfg(enable_avx2)]
pub const PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_BYTES: usize = 64;
...
#[cfg(enable_avx2)]
#[link(name = "rainbowIa-cyclic_avx2")]
extern "C" {
    #[cfg(enable_avx2)]
    pub fn PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_keypair(pk: *mut u8, sk: *mut u8) -> c_int;
    #[cfg(enable_avx2)]
    pub fn PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign(
        sm: *mut u8,
        smlen: *mut usize,
        msg: *const u8,
        len: usize,
    ...
    #[cfg(enable_avx2)]
    pub fn PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_open(
        m: *mut u8,
        mlen: *mut usize,
        sm: *const u8,
        smlen: usize,
    ...
    #[cfg(enable_avx2)]
    pub fn PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_signature(
        sig: *mut u8,
        siglen: *mut usize,
        m: *const u8,
        mlen: usize,
    ...
    #[cfg(enable_avx2)]
    pub fn PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_verify(
        sig: *const u8,
        siglen: usize,
        m: *const u8,
        mlen: usize,
...
mod test_rainbowiacyclic_avx2 {
    ...
    unsafe fn run_test_ffi() {
        ...
        let mut pk = vec![0u8; PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_PUBLICKEYBYTES];
        let mut sk = vec![0u8; PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_SECRETKEYBYTES];
        let mut pk_alt = vec![0u8; PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_PUBLICKEYBYTES];
        let mut sk_alt = vec![0u8; PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_SECRETKEYBYTES];
        let mut detached_sig = vec![0u8; PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_BYTES];
        let mut sm = Vec::with_capacity(mlen + PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_BYTES);
        let mut smlen = 0;
        assert_eq!(
            0,
            PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_keypair(pk.as_mut_ptr(), sk.as_mut_ptr())
        );
        assert_eq!(
            0,
            PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign(
                sm.as_mut_ptr(),
                &mut smlen as *mut usize,
                msg.as_ptr(),
                mlen,
        ...
        let mut unpacked_m = Vec::with_capacity(mlen + PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_BYTES);
        assert_eq!(
            0,
            PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_open(
                unpacked_m.as_mut_ptr(),
                &mut mlen as *mut usize,
                sm.as_ptr(),
                sm.len(),
        ...
        assert_eq!(
            0,
            PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_keypair(
                pk_alt.as_mut_ptr(),
                sk_alt.as_mut_ptr()
            )
        );
        ...
        assert_eq!(
            0,
            PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_signature(
                detached_sig.as_mut_ptr(),
                &mut smlen as *mut usize,
                msg.as_ptr(),
                msg.len(),
        ...
        assert!(
            smlen <= PQCLEAN_RAINBOWIACYCLIC_AVX2_CRYPTO_BYTES,
            "Signed message length should be ≤ CRYPTO_BYTES"
        );
        assert_eq!(
            0,
            PQCLEAN_RAINBOWIACYCLIC_AVX2_crypto_sign_verify(
                detached_sig.as_ptr(),
                smlen,
                msg.as_ptr(),
                msg.len(),
